...
public class PopupEnumBhv : PopupBhv
{
    ...
    public void Init<EnumType>(string title, int currentId, System.Func<int, object> resultAction, int startId) where EnumType : System.Enum
    {
        ...
        int y = 0;
        for (int i = startId; i < values.Length; ++i)
        {
            var tmpButtonObject = Resources.Load<GameObject>("Prefabs/EnumButton");
            var tmpButtonInstance = Instantiate(tmpButtonObject, _listStartPosition + new Vector3(0.0f, -spaceBetween * y, 0.0f), tmpButtonObject.transform.rotation);
